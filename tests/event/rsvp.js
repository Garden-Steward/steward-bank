...
describe('getRSVPs', function() {
  ...
  it('should invite user to join the garden', async function() {
    const eventMock = require('./eventMock.js');
    eventMock.event.garden = {id: 1, title: "Test Garden", sms_slug: "test-garden"};
    eventMock.event.id = "39";
    const noconfirmedEvent = eventMock.event;
    noconfirmedEvent.confirmed = [];    
    await strapi.db.query('api::volunteer-day.volunteer-day').create({data: noconfirmedEvent});
    eventHelper.inviteUserEvent("39", {phoneNumber: "1234567890"}).then(res=>{
      const {body} = res;
      expect(body).toBe("Hello! Do you have interest in volunteering for Test Event? To start, reply with test-garden. We'll walk you through a quick registration.");
    });
  });
});
